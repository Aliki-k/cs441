...
public class SignUpReceptionist extends JFrame implements ActionListener{
		private Start start;
		 ...
		 * @param start 
		 */
		public SignUpReceptionist(Start start) {
			this.start=start;
			...
			b=new JButton("Sign up as Receptionist");
			b.setBounds(193, 442, 200, 40);
		...
		if(e.getSource()==b) {
			...
			if(name.length()==0||surname.length()==0||email.length()==0||telephone.length()==0||id.length()==0||address.length()==0||username.length()==0||pass1.length()==0||pass2.length()==0) 
				...
				try {
					...
					boolean b=stub.signUp(Integer.parseInt(id), name, surname, Integer.parseInt(telephone), email, username, pass1, address, 2);
					if(b)
