...
public class SignUpClinicalStaff extends JFrame implements ActionListener{
	private Start start;
	...
	public SignUpClinicalStaff(Start start) {
		...
		b=new JButton("Sign up as Clinical staff");
		...
		if(e.getSource()==b) {
			...
			if(name.length()==0||surname.length()==0||email.length()==0||telephone.length()==0||id.length()==0||address.length()==0||username.length()==0||pass1.length()==0||pass2.length()==0) 
				...
				try {
					...
					boolean b=stub.signUp(Integer.parseInt(id), name, surname, Integer.parseInt(telephone), email, username, pass1, address, 1);
					if(b)
